...
import { Save, RefreshCw, Smile, ShieldAlert, HeartPulse, Activity, UserPlus, Sparkles, Mic, MicOff, Volume2, VolumeX, Check, ChevronDown, ChevronUp, AlertCircle, X } from 'lucide-react';
import { RespondentData, DeciduousTeethState, PermanentTeethState } from '../types';
import Odontogram from './Odontogram';

interface DentalFormProps {
  onSaveRespondent: (data: Omit<RespondentData, 'id' | 'createdAt' | 'createdBy'>, editId?: string) => Promise<void>;
...
export default function DentalForm({ onSaveRespondent, nextRespondentNumber, editingRespondent, onCancelEdit }: DentalFormProps) {
  // Identitas
  ...
  const STATUS_KEYWORDS = [
    ...
    { phrase: 'sudah tanggal', codePerm: '5', codeDecid: 'K' },
    { phrase: 'tanggal', codePerm: '5', codeDecid: 'K' },
    { phrase: 'kode k', codePerm: '5', codeDecid: 'K' }
  ];
  ...
  // Load editingRespondent values if in edit mode
  useEffect(() => {
    if (editingRespondent) {
      setNama(editingRespondent.nama);
      setAlamat(editingRespondent.alamat || '');
      setNoTelepon(editingRespondent.noTelepon || '');
      setNamaOrangTua(editingRespondent.namaOrangTua || '');
      setTanggalPemeriksaan(editingRespondent.tanggalPemeriksaan || new Date().toISOString().split('T')[0]);
      setUmur(editingRespondent.umur);
      setJenisKelamin(editingRespondent.jenisKelamin);
      setPendidikan(editingRespondent.pendidikan);
      setPekerjaan(editingRespondent.pekerjaan);
      if (editingRespondent.teethStatus) {
        setTeethStatus(editingRespondent.teethStatus);
      }
      setGusiBerdarah(editingRespondent.mukosa.gusiBerdarah);
      setLesiMukosaOral(editingRespondent.mukosa.lesiMukosaOral);
      setPerluPerawatanSegera(editingRespondent.tindakLanjut.perluPerawatanSegera);
      setPerluPerawatanTidakSegera(editingRespondent.tindakLanjut.perluPerawatanTidakSegera);
      setPerluDirujuk(editingRespondent.tindakLanjut.perluDirujuk);
      setDirujukKe(editingRespondent.tindakLanjut.dirujukKe);
    } else {
      resetForm();
    }
  }, [editingRespondent]);

  // Set default nama based on respondent number (only if not editing)
  useEffect(() => {
    if (!nama && !editingRespondent) {
      setNama(`Responden #${nextRespondentNumber}`);
    }
  }, [nextRespondentNumber, editingRespondent]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const respondentPayload: Omit<RespondentData, 'id' | 'createdAt' | 'createdBy'> = {
        ...
        tanggalInput: editingRespondent?.tanggalInput || new Date().toISOString().split('T')[0],
        tanggalPemeriksaan,
        jenisKelamin,
        umur,
      ...
      await onSaveRespondent(respondentPayload, editingRespondent?.id);
      setSuccess(true);
      resetForm();
      setTimeout(() => {
        setSuccess(false);
    ...
    <form onSubmit={handleSubmit} className="space-y-6 max-w-5xl mx-auto pb-12 p-2" id="dental-form-root">
        ...
        <div className="space-y-6 lg:col-span-1">
          ...
          <div className="glass-panel rounded-3xl border border-white/40 p-5 shadow-md space-y-4">
            ...
            <div className="space-y-3.5">
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Tanggal Pemeriksaan</label>
                <input
                  type="date"
                  required
                  value={tanggalPemeriksaan}
                  onChange={e => setTanggalPemeriksaan(e.target.value)}
                  className="w-full px-3.5 py-2.5 glass-input rounded-xl text-slate-800 text-sm focus:outline-none transition-all font-bold"
                />
              </div>
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Alamat</label>
                <input
                  type="text"
                  id="alamat-input"
                  value={alamat}
                  ...
                  className="w-full px-3.5 py-2.5 glass-input rounded-xl text-slate-800 text-sm focus:outline-none transition-all placeholder-slate-400 font-medium"
                  placeholder="Alamat Lengkap Responden"
                />
              </div>

              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">No. Telepon</label>
                <input
                  type="tel"
                  id="no-telepon-input"
                  value={noTelepon}
                  onChange={e => setNoTelepon(e.target.value)}
                  className="w-full px-3.5 py-2.5 glass-input rounded-xl text-slate-800 text-sm focus:outline-none transition-all placeholder-slate-400 font-medium font-mono"
                  placeholder="Contoh: 081234567890"
                />
              </div>

              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5">Nama Orang Tua</label>
                <input
                  type="text"
                  id="nama-orangtua-input"
                  value={namaOrangTua}
            ...
            <div className="glass-panel rounded-3xl border border-white/40 p-5 shadow-md space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="border-t border-white/20 my-3 pt-3">
                  ...
                  <ToothAdjuster label="Gigi Sudah Tanggal (K)" type="sulung" field="sudahTanggal" value={gigiSulung.sudahTanggal || 0} />
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-5 glass-panel border border-white/40 rounded-3xl shadow-lg">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          {editingRespondent && onCancelEdit && (
            <button
              type="button"
              onClick={onCancelEdit}
              className="flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4.5 py-3 bg-rose-50 hover:bg-rose-100 text-rose-700 text-xs font-bold rounded-xl border border-rose-200/50 shadow-xs cursor-pointer transition-all"
            >
          ...
          >
            ...
            {saving ? 'Menyimpan...' : (editingRespondent ? 'Simpan Perubahan' : 'Simpan Data')}
          </button>
        </div>
      </div>
